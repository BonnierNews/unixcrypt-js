"use strict";
// JS version of https://github.com/markusberg/unixcrypt
...
function generateDigestA(plaintext, conf) {
  ...
  // steps 1-8
  const hashA = crypto.createHash(HashType[conf.id]);
  hashA.update(plaintext);
  hashA.update(conf.saltString);
  const hashB = crypto.createHash(HashType[conf.id]);
...
function generateHash(plaintext, conf) {
  ...
  // steps 13-15
  const plaintextByteLength = Buffer.byteLength(plaintext);
  const hashDP = crypto.createHash(hashType);
  ...
  // step 17-19
  const hashDS = crypto.createHash(hashType);
  ...
  const digestC = rounds.reduce((acc, curr, idx) => {
    const hashC = crypto.createHash(hashType);
    // steps b-c
    if (idx % 2 === 0) {
      hashC.update(acc);
    } else {
...
function base64Encode(digest, specificShuffleMap) {
  let hash = "";
  for (let idx = 0; idx < digest.length; idx += 3) {
    const buf = Buffer.alloc(3);
    buf[0] = digest[specificShuffleMap[idx]];
    buf[1] = digest[specificShuffleMap[idx + 1]];
    buf[2] = digest[specificShuffleMap[idx + 2]];
